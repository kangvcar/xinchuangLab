...
exec ttyd \
  ...
  su - student -c "LINUX_AI_SESSION_DIR=/tmp/linux-ai-session tmux -f /opt/linux-ai/tmux.conf new-session -A -s linux-ai-lab 'bash --rcfile /opt/linux-ai/bash-instrumentation.sh -i'"
